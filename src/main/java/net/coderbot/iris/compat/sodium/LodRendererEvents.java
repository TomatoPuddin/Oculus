...
public class LodRendererEvents {
        ...
        if (!eventHandlersBound) {
            ...
            DhApiAfterDhInitEvent beforeCleanupEvent = new DhApiAfterDhInitEvent() {
                ...
                public void afterDistantHorizonsInit(DhApiEventParam<Void> event) {
                    ...
                    net.coderbot.iris.compat.dh.DHCompatInternal.dhEnabled = DhApi.Delayed.configs.graphics().renderingEnabled().getValue();
    ...
    private static net.coderbot.iris.compat.dh.DHCompatInternal getInstance() {
        return (net.coderbot.iris.compat.dh.DHCompatInternal) Iris.getPipelineManager().getPipeline().map(WorldRenderingPipeline::getDHCompat).map(DHCompat::getInstance).orElse(DHCompatInternal.SHADERLESS);
    }
    ...
    private static void beforeBufferRenderEvent() {
        DhApiBeforeBufferRenderEvent beforeCleanupEvent = new DhApiBeforeBufferRenderEvent() {
            ...
            public void beforeRender(DhApiEventParam<EventParam> input) {
                net.coderbot.iris.compat.dh.DHCompatInternal instance = getInstance();
                    ...
                    if (instance.shouldOverride) {
                        if (ShadowRenderingState.areShadowsCurrentlyBeingRendered()) {
                            instance.getShadowShader().fillUniformData(
                                    ((Matrix4fAccess) (Object) ShadowRenderer.PROJECTION).convertToJOML(), ((Matrix4fAccess) (Object) ShadowRenderer.MODELVIEW).convertToJOML(),
                                    -1000, //MC.getWrappedClientLevel().getMinHeight(),
                                    partialTicks);
                        } else {
                            Matrix4f projection = ((Matrix4fAccess) (Object) CapturedRenderingState.INSTANCE.getGbufferProjection()).convertToJOML();
                            ...
                            instance.getSolidShader().fillUniformData(
                                    ...
                                    ((Matrix4fAccess) (Object) CapturedRenderingState.INSTANCE.getGbufferModelView()).convertToJOML(),
                                    -1000, //MC.getWrappedClientLevel().getMinHeight(),
                                    partialTicks);
                        }
                    }
